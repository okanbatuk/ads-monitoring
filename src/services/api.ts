...
const API_BASE_URL = "https://ads-script-api.onrender.com/api";
    ...
    queryFn: async (): Promise<SubAccountsApiResponse> => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/accounts/${accountId}/accounts?include=true`,
        );
        if (!response.ok) {
        ...
        console.error(
          `[useSubAccounts] Error fetching sub-accounts for ${accountId}:`,
          error,
        );
        throw error;
      }
  ...
  return useQuery<GetAccountResponse>({
    ...
    queryFn: async () => {
      try {
        ...
        console.error(
          `[useAccount] Error fetching account ${accountId}:`,
          error,
        );
        throw error;
      } finally {
  ...
  return useQuery<GetAccountCampaignsResponse>({
    ...
    queryFn: async () => {
      try {
        ...
        console.error(
          `[useAccountCampaigns] Error fetching campaigns for account ${accountId}:`,
          error,
        );
        throw error;
      } finally {
...
export const useBulkAccountScores = () => {
  return useMutation<
    PostAccountBulkScoresResponse,
    Error,
    PostAccountBulkScoresRequest & { days: number }
  >({
    mutationFn: ({ ids, days }) =>
      fetchApi<PostAccountBulkScoresResponse>(
        `/accounts/bulkscores?days=${days}`,
        {
          method: "POST",
...
export const useCampaignScores = (campaignId: string, days: number) => {
  return useQuery<GetCampaignScoreResponse>({
    ...
    queryFn: () =>
      fetchApi<GetCampaignScoreResponse>(
        `/campaigns/${campaignId}/scores?days=${days}`,
      ),
...
export const useCampaignAdGroups = (campaignId: string) => {
  return useQuery<GetCampaignAdGroupsResponse>({
    ...
    queryFn: () =>
      fetchApi<GetCampaignAdGroupsResponse>(
        `/campaigns/${campaignId}/adgroups?include=true`,
      ),
    enabled: !!campaignId,
  });
};
...
export const useBulkCampaignScores = () => {
  return useMutation<
    PostCampaignBulkScoresResponse,
    Error,
    PostCampaignBulkScoresRequest & { days: number }
  >({
    mutationFn: ({ ids, days }) =>
      fetchApi<PostCampaignBulkScoresResponse>(
        `/campaigns/bulkscores?days=${days}`,
        {
          method: "POST",
          body: JSON.stringify({ ids }),
        },
...
export const useKeywordScores = (keywordId: string, days: number) => {
  return useQuery<GetKeywordScoresResponse>({
    ...
    queryFn: () =>
      fetchApi<GetKeywordScoresResponse>(
        `/keywords/${keywordId}/scores?days=${days}`,
      ),
...
export const useAdGroupKeywords = (
  adGroupId: string,
  options: UseAdGroupKeywordsOptions = {},
) => {
  return useQuery<GetKeywordsResponse, Error>({
    ...
    enabled:
      !!adGroupId && (options.enabled !== undefined ? options.enabled : true),
    ...options,
  });
};
