...
export interface UseAccountTreeReturn {
  ...
  loadChildren: (
    node: TreeNode,
    onComplete?: (children: TreeNode[]) => void,
  ) => Promise<void>;
...
export const useAccountTree = (): UseAccountTreeReturn => {
  ...
  const updateNode = useCallback(
    (nodes: TreeNode[], id: string, updates: Partial<TreeNode>): TreeNode[] => {
  ...
  >({});
  const [loadingSubAccounts, setLoadingSubAccounts] = useState<
  ...
  const fetchAccountCampaigns = useCallback(
    async (accountId: string): Promise<TreeNode[]> => {
      try {
        const response = await fetchApi<{ data: { campaigns: CampaignDto[] } }>(
  ...
  const fetchCampaignAdGroups = useCallback(
    async (campaignId: string): Promise<TreeNode[]> => {
      try {
        const response = await fetchApi<{
          data: { adGroups: AdGroupDto[]; total: number };
  ...
  const loadChildren = useCallback(
    async (node: TreeNode, onComplete?: (children: TreeNode[]) => void) => {
      if (!node.hasChildren) {
        return;
      }
      ...
      try {
        ...
        if (node.isMcc || (node.type === "mcc" && !node.parentId)) {
          ...
          const mccId = node.id.startsWith("account-")
  ...
  // Helper function to collect all child node IDs recursively
  const collectChildNodeIds = useCallback(
    (node: TreeNode, ids: Set<string> = new Set()) => {
      if (node.children) {
    ...
    async (nodeId: string) => {
      ...
      setExpandedNodes((prev) => {
        ...
        if (newExpandedNodes.has(nodeId)) {
          // Node is being collapsed - remove this node and all its children from expanded nodes
          newExpandedNodes.delete(nodeId);

          // If node has children, remove them from expanded nodes as well
          if (node.children && node.children.length > 0) {
            const childIds = collectChildNodeIds(node);
            childIds.forEach((id) => newExpandedNodes.delete(id));
          }
        } else {
          // Node is being expanded - just add this node
      ...
      // Only load children if we're expanding the node
